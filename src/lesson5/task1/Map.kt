...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((key, value) in grades) {
        if (map[value] != null) map[value]!!.add(key)
        else map[value] = mutableListOf(key)
    }
    return map
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((key, value) in stockPrices)
        if (groups[key] != null) groups[key]!!.add(value)
        else groups[key] = mutableListOf(value)
    for ((key, value) in groups) {
        avg[key] = value.sum() / value.size
    }
    return avg
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    var res: String? = null
    var counter = 10000000000000.0
    for ((key, value) in stuff) {
        if (value.first == kind && value.second < counter) {
            res = key
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val lowerchars = chars.toString().toLowerCase().toSet()
    val lowerword = word.toLowerCase().toSet()
    var cnt = 0
    for (element in lowerword) {
        if (element in lowerchars) cnt += 1
    }
    if (lowerword.size != cnt) return false
    return true
}
...
fun hasAnagrams(words: List<String>): Boolean {
    for (wrd in words) {
        for (checkwrd in words) {
            if (checkwrd != wrd && checkwrd.length == wrd.length) {
                var cnt = 0;
                for (lttr in wrd) {
                    if (lttr in checkwrd) cnt += 1
                }
                if (cnt == wrd.length) return true
            }
        }
    }
    return false
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    val TreasureTypes = mutableListOf<String>()
    val prices = Array(treasures.size + 1) { Array(capacity + 1) { 0 } }
    for ((key, value) in treasures) {
        Values.add(value.second)
        Weigths.add(value.first)
        TreasureTypes.add(key)
    }
    for (i in 1..treasures.size)
        for (j in 0..capacity)
            if (j >= Weigths[i - 1])
                prices[i][j] = max(prices[i - 1][j], prices[i - 1][j - Weigths[i - 1]] + Values[i - 1])
            else
